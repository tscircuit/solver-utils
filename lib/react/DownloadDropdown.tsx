...
export const DownloadDropdown = ({
  ...
  const downloadJSON = () => {
    try {
      if (typeof solver.getConstructorParams !== "function") {
        alert(
          `getConstructorParams() is not implemented for ${getSolverName(solver)}`,
        )
        return
      }
      ...
      a.href = url
      a.download = `${getSolverName(solver)}_params.json`
      a.click()
      URL.revokeObjectURL(url)
    } catch (error) {
      alert(
        `Error downloading params for ${getSolverName(solver)}: ${error instanceof Error ? error.message : String(error)}`,
      )
    }
    setIsOpen(false)
  }
  ...
  const downloadPageTsx = () => {
    try {
      ...
      const solverName = getSolverName(solver)
...
`
      ...
      alert(
        `Error generating page.tsx for ${getSolverName(solver)}: ${error instanceof Error ? error.message : String(error)}`,
      )
    }
    setIsOpen(false)
  }
...
`
      ...
      alert(
        `Error generating test.ts for ${getSolverName(solver)}: ${error instanceof Error ? error.message : String(error)}`,
      )
    }
    setIsOpen(false)
  }
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        title={`Download options for ${getSolverName(solver)}`}
      >
